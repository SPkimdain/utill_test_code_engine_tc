...
public final class SecurityArgumentConstructorThread {
    private final ExecutorService pool;

    public SecurityArgumentConstructorThread(int poolSize) {
    ...
    public StringBuilder fieldAggregator(String... inputs)
        ...
        for (int i = 0; i < inputs.length; i++) {
            results[i] = pool.submit(new ValidateInput<String>(inputs[i], pool));   // @violation
        }
