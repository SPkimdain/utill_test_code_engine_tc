...
public class WeakCryptographicHashHardcodedSalt {
    ...
    public byte[] test(byte messageByte, byte[] messageBytes, String message) throws Exception {
        ...
        messageDigest.update((byte)12); // @violation
        ...
        messageDigest.update(unsafeSaltByte); // @violation
        unsafeSaltByte=getB();
        messageDigest.update(unsafeSaltByte);

        messageDigest.update(safeSaltByteArray);
        messageDigest.update(unsafeSaltByteArray1); // @violation
        messageDigest.update(unsafeSaltByteArray1, n1, n2); // @violation

        messageDigest.update(new String(unsafeSaltByteArray2).getBytes());
        messageDigest.update(new byte[]{12, 1}); // @violation
        messageDigest.update(unsafeSaltByteArray2); // @violation
